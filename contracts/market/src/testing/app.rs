use super::scenario::{
    ADMIN, ASSET_DENOM, STABLE_DENOM, TRADER1, TRADER2, TRADER3, TRADER4, TRADER5,
};
...
use cosmwasm_std::{
    ...
    Addr, Api, Attribute, Empty, GovMsg, IbcMsg, IbcQuery, Response, StdError, Storage,
    SubMsgResult,
};
use cw_multi_test::{
...
pub fn init_default_balances(
    ...
    router
        ...
        .init_balance(
            ...
            vec![
                coin(100_000_000, ASSET_DENOM.to_string()),
                coin(100_000_000, STABLE_DENOM.to_string()),
            ],
        )
        .unwrap();
...
pub fn setup_test(
            ...
            1 => match msg.result {
                SubMsgResult::Ok(res) => match res.data {
                    Some(data) => {
                        ...
                        let market_contract_addr = String::from_utf8(string_data.to_vec()).unwrap();
    ...
    let msg = PoolQueryMsg::GetConfig {};
